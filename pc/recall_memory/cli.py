...
def _open(args) -> tuple[MemoryStore, RecallConfig]:
    cfg = RecallConfig(db_path=args.db, embedder=args.embedder,
                       use_tier3_planner=getattr(args, "tier3", False))
...
def main(argv=None) -> int:
    ...
    p.add_argument("--embedder", default="auto", choices=["auto", "ollama", "hash"])
    ...
    if args.cmd == "demo":
        ...
        ids = seed(Ingestor(store, cfg, embedder))
        print(json.dumps({"seeded": ids, "stats": store.stats(),
                          "embedder": embedder.name}, indent=2))

    elif args.cmd == "stats":
        print(json.dumps({**store.stats(), "embedder": embedder.name}, indent=2))

    elif args.cmd == "ask":
        r = Retriever(store, cfg, embedder)
        ...
        with open(args.file, encoding="utf-8") as f:
            meeting = json.load(f)
        mid = Ingestor(store, cfg, embedder).ingest_meeting(meeting)
        print(f"ingested meeting -> memory_id {mid}")

    elif args.cmd == "ingest-text":
        with open(args.file, encoding="utf-8") as f:
            content = f.read()
        mid = Ingestor(store, cfg, embedder).ingest_document(
        ...
        c = Consolidator(store, cfg, Ingestor(store, cfg, embedder))
        print(json.dumps(c.run_mvp(), indent=2))
